...
- ~~Backups automáticos de base de datos~~ — **Resuelto (26 de agosto de 2026).** `backup_db.py` reescrito para detectar el motor y usar `pg_dump` en Postgres (§4.6), timer instalado y verificado en el VPS con un backup real (`pg_restore --list` confirmó las 5 tablas y sus datos). Sigue faltando la **copia off-site** (protege contra migración/deploy que rompa datos, no contra perder el VPS entero) — agregar si/cuando se justifique el costo y complejidad extra. **Disco del VPS: 467 MB libres (91% usado)** a esa fecha — sigue ajustado, vale la pena revisarlo de nuevo antes de cargar muchas imágenes desde el panel.
